...
const handlePostHtml = async function(ctx) {
  ...
  if (!config.insertJsToNextPage.disable) {
    ...
    if (!nextLink) {
      log('所有文章已经抓取完毕');
      // 此时，向redis发送个信号说明公众号爬去，供python脚本读取
      setTimeout(function(){
         redis('set', 'post_finish', 1)
      },10000);
      log();
    } else {
...
async function getNextProfileLink() {
    ...
    // 没有拿到链接则从数据库中查
    if (config.read_mongo_count == 1) return; //mongo读过了，不读了
    ...
    config.read_mongo_count += 1 //mongo读过了，+1, 下次判断不再读
    // 如果还查不到 则证明已经抓取完毕了 返回undefined
    // if (links.length === 0) return;
